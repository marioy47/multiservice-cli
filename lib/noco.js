...
const noco = async (table, query, { format }) => {
  ...
  switch (table.trim().toLowerCase()) {
    case "accounts":
      values = await queryAccounts(nocoToken, query);
      break;
    case "websites":
      values = await queryWebsites(nocoToken, query);
      break;
    default:
  }
...
const queryAccounts = async (nocoToken, query) => {
...
const queryWebsites = async (nocoToken, query) => {
